...
namespace WebApp_Tests
{
    public class Tests
    {
        ...
        public async Task Should_Return_Program_Info_With_OK_StatusCode()
        {
            ...
            Assert.IsNotEmpty(responseBody, "get request Status/ has failed!");
        }
        ...
        public async Task Should_Return_OK_StatusCode_When_Number_Is_Prime()
        {
            ...
            var response = await _httpClient.GetAsync("/Primes/primes/7");
            var responseBody = await response.Content.ReadAsStringAsync();
        ...
        {
            var response = await _httpClient.GetAsync($"Primes/primes/4");
            var responseBody = await response.Content.ReadAsStringAsync();
            ...
            Assert.AreEqual(HttpStatusCode.NotFound, response.StatusCode, "get request Primes/primes/7 has failed!");
        }

        [Test]
        public async Task Should_Return_Not_Empty_Primes_List_With_StatusCode_OK()
        {
            var response = await _httpClient.GetAsync("Primes/primes?from=1&to=5");
            ...
            Assert.AreEqual("[2,3,5]", responseBody, "request Primes/primes?from=1&to=5 failed!");
        }

        [Test]
        public async Task Should_Return_Empty_Primes_List_With_StatusCode_OK()
        {
            var response = await _httpClient.GetAsync("Primes/primes?from=-5&to=1");
        ...
        {
            var response = await _httpClient.GetAsync("Primes/primes?from=-5&to=abc");
